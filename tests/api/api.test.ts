...
import { createOrderDonateMock, createOrderItemMock, updateOrderMock } from './ordersMock';
...
describe('createOrderItemMock', () => {
  ...
  it('debería lanzar un error si newOrderDonate está vacío o no se proporciona', async () => {
    await expect(createOrderItemMock({})).rejects.toThrow("Error al crear el nuevo Order Item: newOrderDonate es requerido");
    await expect(createOrderItemMock()).rejects.toThrow("Error al crear el nuevo Order Item: newOrderDonate es requerido");
  });
});
